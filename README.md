...
Requires Python 3.10+. Uses Docker if it's running; otherwise a native authorizer binary, installed automatically.
...
`bootstrap` writes a starter `tenuo.yaml`, starts the authorizer, and runs a self-test (`verify`). The starter policy is deliberately strict:
...
Each constraint checks one argument of the tool call:
...
| `shlex:a,b,c` | Bash | the command's **executable** must be one of `a,b,c`, and the command must be a single simple command — pipes, `&&`/`;` chaining, subshells, and shell expansion are rejected. (This allowlists the *verb*, not file paths: `cat /etc/passwd` passes if `cat` is allowed. Use `Read`/`Write` to scope files; drop `Bash` for a hard lock.) |
| `domains` / `cidrs` | WebFetch | the URL's host must match an allowed domain (`*` matches one label) or CIDR range, **and** pass SSRF hygiene — https-only by default, with loopback, cloud-metadata IPs, encoded-IP tricks, and spoofed hosts (`api.github.com.evil.com`) blocked. |
| `oneof:a,b` · `exact:v` · `pattern:glob` · `regex:re` | any tool argument | the value must be in the set / equal / glob-match / regex-match. |
...
|---------|--------------|
| `onboard` | **Start here.** Interactive first-run setup (`--local` / `--cloud`); scaffolds an example policy if you don't have one. |
| `bootstrap` | Non-interactive first run: scaffold starter policy (if none) → `init` → `up` → `verify`. `--cloud` for Cloud. |
